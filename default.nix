...
let
  ...
  fromManifest' =
          ...
          mapAttrs
            ...
            (
              filterAttrs (_: src: src ? available && src.available) (
                mapAttrs (_: pkg: pkg.target."*" or pkg.target.${target} or null) manifest.pkg
              )
            );
      };
    in
    ...
    // mapAttrs' (
      k: v:
      nameValuePair "${k}Toolchain" (
        toolchain.withComponents (filter (component: toolchain ? ${component}) v)
      )
    ) manifest.profiles
    // {
      inherit manifest;
  ...
  toolchainOf' =
    ...
    in
    fromManifestFile' target "-${channel}" (
      if (sha256 == null) then builtins.fetchurl url else pkgs.fetchurl { inherit url sha256; }
    );
  ...
  fromToolchainName' =
    ...
    mapNullable
      ...
      (
        match "^(stable|beta|nightly|[[:digit:]]+\.[[:digit:]]+(\.[[:digit:]]+)?)(-([[:digit:]]{4}-[[:digit:]]{2}-[[:digit:]]{2}))?(-([-[:alnum:]]+))?\n?$" name
      );
  ...
  fromToolchainFile' =
    ...
    let
      text = readFile (
        if file == null && dir != null then
          findFirst pathIsRegularFile (throw "No rust toolchain file found in ${dir}") [
            (dir + "/rust-toolchain")
    ...
    if toolchain == null then
      ...
      if t ? path then
        ...
        in
        combine' "rust-${t.channel}" (
          attrVals (filter (component: toolchain ? ${component}) (
            unique (toolchain.manifest.profiles.${t.profile or "default"} ++ t.components or [ ])
          )) toolchain
          ++ map (target: (fromManifest' target "-${t.channel}" toolchain.manifest).rust-std) (
            t.targets or [ ]
          )
        )
    else
  ...
  mkToolchains =
    channel:
    let
      manifest = importJSON (./data + "/${channel}.json");
    in
    mapAttrs (target: _: {
      ${channel} = fromManifest' target "-${channel}" manifest;
...
// rec {
  ...
  fromToolchainName =
    ...
    fromToolchainName' v name sha256;
